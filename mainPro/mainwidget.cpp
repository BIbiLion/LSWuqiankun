 ...
 void MainWidget::createConnect()
{
   ...
   connect(ui->page1_cacle_btn,SIGNAL(clicked()),this,SLOT());
    ...
    //page3
     connect(ui->page3_sure_Btn,SIGNAL(clicked()),this,SLOT(hide()));
      ...
      while(i < _fileList.count() )
      {
        ...
        QString _newFile=m_saveDir+"\\"+_fileInfo.fileName();
         ...
         bool _isOk=QFile::copy(_fileList[i],_newFile);
          qDebug()<<_fileInfo.fileName()<<"  "<<_isOk<<"new FileName:"<<_newFile;
         ++i;
      }
...
#include "module.h"
   ...
   void MainWidget::mouseMoveEvent(QMouseEvent * mouseEvent)
   {
    ...
    Module::getInstance().instllModule();
    Module::getInstance().removeModule();

       if(m_isMove)
       {
           QPoint move_pos = mouseEvent->globalPos();
           this->move(move_pos - m_point);
       }
   }
